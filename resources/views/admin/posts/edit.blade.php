    ...
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data">
        ...
        <div class="card p-4 p-md-5 mb-4">
            ...
            @php
                ...
                $selectedTags = collect(old('tags', $post->tags->pluck('id')->all()));
            @endphp
        ...
        <div class="card p-4 p-md-5 mb-4">
            ...
            @if ($post->image)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="admin-image-preview">
                </div>
            @endif

            <label for="image" class="form-label fw-semibold">{{ $post->image ? 'Replace Image' : 'Upload Image' }}</label>
            <input
                type="file"
                name="image"
                id="image"
